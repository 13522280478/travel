...
//        String strPattern = "^[a-zs-9]+([._\\\\-]*[a-zs-9])*@([a-zs-9]+[-a-zs-9]*[a-zs-9]+.){1,63}[a-zs-9]+$";
//        String strPattern = "^([a-zs-9A-Z]+[-|\\.]?)+[a-zs-9A-Z]@([a-zs-9A-Z]+(-[a-zs-9A-Z]+)?\\.)+[a-zA-Z]{2,}$";
...
//        return b;
    ...
    public static String getModleTime(String time) {
        if (StringUtils.isEmpty(time) || time.length() < 12) {
            return "";
        }
        String mTime = null;
        String year = time.substring(0, 4);
